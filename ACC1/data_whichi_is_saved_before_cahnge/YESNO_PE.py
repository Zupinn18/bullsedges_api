...
data_file_path = "46832PE.csv"
graph_file_path = "46832PE.html"
...
def feed_data(message):
    ...
    if feed_message["t"] == "ck":
        ...
        if 'lp' in feed_message:
            ...
            # collection.insert_one(feed_message)  # Uncomment this line if 'collection' is defined
...
# Function to generate subscribe_list based on user input
def get_subscribe_list(instrument_type, token):
    return [alice.get_instrument_by_token(instrument_type, token)]
...
# Take user inputs
instrument_type = input("Enter instrument type (e.g., NFO): ")
token = int(input("Enter token (e.g., 46832): "))

# Subscribe to user-specified instrument
subscribe_list = get_subscribe_list(instrument_type, token)
alice.subscribe(subscribe_list)
...
def calculate_heikin_ashi(data):
    ...
    label_csv_filename = 'label_46832PE_02.csv'
    try:
...
# Read the 'label_46832CE_17.csv' and 'band_data.csv' files
