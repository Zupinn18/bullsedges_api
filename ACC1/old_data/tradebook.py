...
data_file_path = "1375hcc.csv"

graph_file_path = "1375hcc.html"
...
subscribe_list = [alice.get_instrument_by_token('NSE', 1375)]
alice.subscribe(subscribe_list)
print(datetime.now())
sleep(10)
print(datetime.now())
...
def calculate_heikin_ashi(data):
    ...
    for i in range(1, len(ha_data)):
        ...
        if ha_data['close'].iloc[i - 1] > ha_data['open'].iloc[i - 1] and ha_data['close'].iloc[i] > ha_data['open'].iloc[i]:
            if consecutive_green_candles == 0:
                ...
                trade_book_data = pd.read_csv('trade_book.csv')  # Assuming 'trade_book.csv' contains the trade book data
    ...
    label_csv_filename = 'label_1375_hcc.csv'
    try:
...
def update_label_trade_book(trade_book_data):
    ...
    label_trade_book_filename = 'trade_book.csv'
    try:
...
# Example usage:
# Assuming 'trade_book_data' contains the list of dictionaries provided in the question
