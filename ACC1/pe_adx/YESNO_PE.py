...
data_file_path = "41913PE.csv"

graph_file_path = "41913PE.html"
...
subscribe_list = [alice.get_instrument_by_token('NFO', 41913)]
alice.subscribe(subscribe_list)
print(datetime.now())
sleep(10)
print(datetime.now())
...
def calculate_heikin_ashi(data):
    ...
    for i in range(1, len(ha_data)):  # Start from the second candle to check for green candles
        if ha_data['close'].iloc[i - 1] > ha_data['open'].iloc[i - 1] and ha_data['close'].iloc[i] > ha_data['open'].iloc[i]:
            if consecutive_green_candles == 0:  # Only if not already counting green candles
                ...
                label_data.append(('YES', ha_data.index[i], data['high'].iloc[i], None))
    ...
    label_csv_filename = 'label_41913PE.csv'
    try:
