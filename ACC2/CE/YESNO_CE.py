...
data_file_path = "39172CE.csv"

graph_file_path = "39172CE.html"
...
subscribe_list = [alice.get_instrument_by_token('NFO', 39172)]
alice.subscribe(subscribe_list)
print(datetime.now())
sleep(10)
print(datetime.now())
...
def calculate_heikin_ashi(data):
    ...
    for i in range(1, len(ha_data)):
        ...
        if (ha_data['close'].iloc[i - 1] > ha_data['open'].iloc[i - 1] and
            ...
            if consecutive_green_candles == 1 and ha_data['high'].iloc[i] > ha_data['high'].iloc[i - 1]:
...
# Read the 'label_39172CE_17.csv' and 'band_data.csv' files
