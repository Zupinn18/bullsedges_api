...
data_file_path = "38189CE.csv"

graph_file_path = "38189CE.html"
...
subscribe_list = [alice.get_instrument_by_token('NFO', 38189)]
alice.subscribe(subscribe_list)
print(datetime.now())
sleep(10)
print(datetime.now())
...
def calculate_heikin_ashi(data):
    ...
    # Initialize state
    in_green_candle = False
    in_red_candle = False
    last_no_candle_close = 0
    last_no_candle_high = 0

    # Iterate through the data
    for index, candle in ha_data.iterrows():
        if candle['close'] > candle['open'] and not in_green_candle:
            # First green candle after red candle
            current_label = 'YES'
            in_green_candle = True
            in_red_candle = False
            last_green_candle_close = candle['close']
            if candle['high'] > last_no_candle_high + 5:
                ha_data.at[index, 'mark'] = 'FIVE'
                current_label = "FIVE"
        elif candle['close'] < candle['open'] and not in_red_candle:
            # First red candle after green candle
            current_label = 'NO'
            in_red_candle = True
            in_green_candle = False
            last_no_candle_close = candle['high']
            last_no_candle_high = candle['high']  # Update the high value for the last red candle
        else:
            current_label = 'NA'

        # Check for the condition and update mark
        if current_label in ['YES', 'FIVE'] and candle['high'] > last_no_candle_high:
            ha_data.at[index, 'mark'] = current_label

        # Add label to the label_data list
        label_data.append([current_label, index, candle['close'], candle['close'] - last_no_candle_close])

    label_csv_filename = 'label_39172CE_8.csv'
    try:
...
# Read the 'label_38189CE_17.csv' and 'band_data.csv' files
