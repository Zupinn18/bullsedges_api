...
data_file_path = "64012CE.csv"

graph_file_path = "64012CE.html"
...
subscribe_list = [alice.get_instrument_by_token('NFO', 64012)]
alice.subscribe(subscribe_list)
print(datetime.now())
sleep(10)
print(datetime.now())
...
def calculate_heikin_ashi(data):
    ...
    changed_label_data = []  # Create an empty list to store label change data

    label_data = []  # Create an empty list to store label data

    for i in range(1, len(ha_data)):
        label = ha_data['mark'].iloc[i]

        # Check if the label has changed from 'YES' to 'NO'
        if label == 'NO' and last_label == 'YES':
            # Add 'CHANGED' label
            changed_label_data.append(('CHANGED', ha_data.index[i], None, None))
            print(f"Label changed from 'YES' to 'NO' at {ha_data.index[i]}")

        # Update last_label and last_label_timestamp
        last_label = label
        last_label_timestamp = ha_data.index[i]

    # Add the 'CHANGED' label data to the label_data list
            ...
            if consecutive_green_candles == 1:
                ...
                for j in range(i, len(ha_data)):
                    if ha_data['high'].iloc[j] > ha_data['high'].iloc[j - 1]:
                        ...
                        if prev_green_high is not None and data['high'].iloc[j] > ha_data['high'].iloc[j - 1] + 10:
            ...
            if consecutive_green_candles > 0:
                ...
                if prev_yes_open is not None:
                    if no_confirmed:  # Calculate difference only if "NO" is confirmed
                        ...
                        if ha_data['close'].iloc[j] < ha_data['open'].iloc[j] and pd.to_datetime('now') - ha_data.index[j] < pd.Timedelta(seconds=time_delay_seconds):
                            ha_data.at[ha_data.index[j], 'mark'] = 'NA'
                            label_data.append(('NA', ha_data.index[j], None, None))
                            print("Marking 'NA' as the current green candle changed to red after a delay.")
    ...
    # label_csv_filename = 'label_64012CE_8.csv'
    # try:
    #     with open(label_csv_filename, 'w', newline='') as csv_file:
    #         csv_writer = csv.writer(csv_file)
    #         csv_writer.writerow(['Label', 'Timestamp', 'Value', 'Difference'])
...
# Read the 'label_64012CE_17.csv' and 'band_data.csv' files
...
def plot_candlestick(data):
    ...
    fig.update_layout(
        title="Live Candlestick Graph CE",
        title_x=0.5,
        xaxis_title="Date",
        yaxis_title="Price",
        xaxis_rangeslider_visible=False,
