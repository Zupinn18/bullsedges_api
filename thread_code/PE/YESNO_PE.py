...
data_file_path = "40729PE.csv"

graph_file_path = "40729PE_02.html"
...
subscribe_list = [alice.get_instrument_by_token('NFO', 40729)]
alice.subscribe(subscribe_list)
print(datetime.now())
sleep(10)
print(datetime.now())
    ...
    for i in range(1, len(ha_data)):
        if (ha_data['close'].iloc[i - 1] > ha_data['open'].iloc[i - 1] and
            ...
            if consecutive_green_candles == 1:
                ...
                # Check if the current closing price is 10 points higher than the previous "YES" high
                if prev_yes_open is not None and data['high'].iloc[i] > prev_yes_open + 7:
                ...
                if prev_yes_open is not None:
                    if no_confirmed:  # Calculate difference only if "NO" is confirmed
                       ...
                       # send_telegram_message(bot_token, chat_id, f"🔴 **NO**: Candle at {ha_data.index[i]}")
                        if i + 1 < len(ha_data):
                            next_low = ha_data['low'].iloc[i + 1]
                            if next_low is not None and prev_green_low is not None:
                                if next_low < prev_green_low:
                                    label_data.append(('low', ha_data.index[i + 1], next_low, None))
            ...
            # Check if the red candle goes down by 7 points
            elif ha_data['open'].iloc[i - 1] - ha_data['close'].iloc[i] >= 7:
                label_data.append(('seven_down', ha_data.index[i], ha_data['close'].iloc[i], 7))
                # Optionally, you can add code here to send a notification or perform any other action.
    
    # Calculate the difference and add it to the DataFrame
    ha_data['Difference'] = ha_data['open'] - ha_data['close']

    label_csv_filename = 'label_40729PE_02.csv'
    try:
...
# Read the 'label_40729CE_17.csv' and 'band_data.csv' files
...
def update_graph_callback(interval, chart_type, relayoutData, n_clicks, n, _):
    ...
    if 'xaxis.range' in relayoutData:
        ...
        xaxis_range = [df.index[-1] - pd.Timedelta(minutes=30), df.index[-1]]
